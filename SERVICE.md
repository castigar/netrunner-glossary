...
| 3 | 공식 KO 대비 정규화 편집거리 | **중앙값 ≤ 0.27** | `1 - SequenceMatcher.ratio()` (공백 정규화 후) |
...
> **게이트 3 근거** — 임계 0.27은 추측이 아니라 실측에서 유도했다. 게이트 3은
> "TM만으로 되는 수준을 30% 이겨라"는 뜻이고, 그 기준선은 **0.385**다.
> `0.385 × 0.7 = 0.2695`.
>
...
> 하드 게이트 1(용어 준수율)이 hold-out에서 용어집 품질을 간접 측정한다.
