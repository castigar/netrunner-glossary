...
- **이 브랜치는 origin에 없다. 커밋 10개가 로컬 전용이다.** 푸시가 §4 작업 1이다.
...
- 남은 일은 그 개정을 **코드에 반영**하는 것이다. 게이트를 통과시키는 작업이 아니라 게이트가 무엇을 재는지 고치는 작업이다.
...
| 브랜치 | HEAD | origin | 내용 |
|---|---|---|---|
| **`ooo/ralph-3436c74…`** | **`d100840`** | **✕ 없음** | **최신. 여기서 작업한다.** Gen 5 산출 10커밋, 테스트 939 |
| `main` | `24a2ba4` | ○ | 기획 문서·Seed·골든 셋·이 문서 |
...
### 작업 1 — Gen 5 브랜치를 푸시한다 ★ 최우선

커밋 10개가 로컬에만 있다. 워크트리 삭제 사고를 한 번 겪은 뒤라 미룰 이유가 없다.

```bash
git push -u origin ooo/ralph-3436c74545374d8296a877f5d4589e
```

public repo이므로 밀기 전에 `.env`·토큰·자격증명이 섞이지 않았는지 확인한다.

### 작업 2 — SERVICE.md §5 개정을 코드에 반영한다 ★ 본체
...
3. **초벌 프롬프트에 기호 개수 1:1 제약** — "모든 게임 기호를 보존한다"를 "추가·중복도 금지, 개수까지 1:1"로 좁힌다. 이번에 찾은 **유일한 진짜 에이전트 결함**(과잉 5건)이고 범위가 좁다
...
5. **hold_out 재생성** — EN 개정 조건을 적용해 판본 불일치 10장을 뺀다. 게이트 2의 천장이 100%로 올라가 하드 게이트로 유효해진다. `tests/test_tm_baseline.py`의 베이스라인 감시선이 함께 움직이므로 같이 갱신한다
6. **테스트 픽스처 누출 차단** — 테스트가 저장소 루트 `new_term_candidates.json`에 쓰지 못하게 한다. `e0d23e5`의 `tmp_path` 리다이렉트가 일부만 덮었다

1~3은 게이트를 녹색으로 만들지 못한다. 그게 정상이고, 그 사실을 측정으로 확정하는 것이 산출물이다.
...
v3에서 고칠 곳은 AC6 문언과 관련 제약이고, 나머지 AC 7개와 goal은 그대로 둔다. AC는 8개로 고정한다.

### 작업 4 — 브랜치 정리

로컬 브랜치 11개 중 6개가 역할이 끝났다(§1 표). Gen 5 푸시 후 정리한다.
