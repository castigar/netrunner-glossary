...
def load_judgments(checkpoint_path: str | Path) -> dict[tuple[str, str], TermJudgment]:
    ...
    with path.open(encoding="utf-8") as fh:
        for line in fh:
            ...
            if not line:
                continue
            try:
                judgment = TermJudgment(**json.loads(line))
            except Exception:
                continue
...
def _append_judgments(checkpoint_path: Path, judgments: list[TermJudgment]) -> None:
    """Append a chunk's judgments to the checkpoint and flush to disk."""
    checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
    with checkpoint_path.open("a", encoding="utf-8") as fh:
        for judgment in judgments:
            fh.write(json.dumps(judgment.model_dump(), ensure_ascii=False) + "\n")
        fh.flush()
...
def judge_candidates(
    ...
    Args:
        ...
        on_progress:     Called once per chunk with a stats dict: ``done``,
                         ``total``, ``accepted``, ``rejected``, ``dropped`` and
                         ``errors`` (exception class name -> count).
    ...
    chain = build_judge_chain(llm).with_retry(stop_after_attempt=3)

    judged: dict[tuple[str, str], TermJudgment] = (
        load_judgments(checkpoint_path) if checkpoint_path else {}
    )
    ...
    for start in range(0, len(pending), chunk_size):
        ...
        if checkpoint_path and fresh:
            _append_judgments(Path(checkpoint_path), fresh)
